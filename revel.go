...
const (
	REVEL_IMPORT_PATH = "github.com/dicefm/revel"
)
...
func findSrcPaths(importPath string) (revelSourcePath, appSourcePath string) {
	...
	appPkg, err := build.Import(importPath, wd, build.FindOnly)
	if err != nil {
		ERROR.Fatalln("Failed to import", importPath, "with error:", err)
	}

	revelPkg, err := build.Import(REVEL_IMPORT_PATH, wd, build.FindOnly)
	if err != nil {
...
func ResolveImportPath(importPath string) (string, error) {
	...
	modPkg, err := build.Import(importPath, wd, build.FindOnly)
	if err != nil {
		return "", err
	}
...
func addModule(name, importPath, modulePath string) {
	...
	if importPath == Config.StringDefault("module.testrunner", "github.com/dicefm/revel/modules/testrunner") {
